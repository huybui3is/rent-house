...
$max_price       = "";
$warning         = "";
$address         = ""; // biến địa chỉ chi tiết (mặc định rỗng)
...
if ($searchTriggered) {
    ...
    // Nếu submit form mà không nhập thông tin nào thì thông báo (nhưng vẫn load trang với kết quả mặc định)
...
// Xây dựng mệnh đề WHERE cơ bản
$where = "WHERE f.TTP_MA = '01' AND f.LS_NGAYKETTHUC IS NULL AND e.is_delete = 0";
$params = [];

// Nếu form được submit và có nhập thông tin tìm kiếm, thì áp dụng bộ lọc
...
if ($searchTriggered && !empty($latitude) && !empty($longitude) && !empty($distance_search)) {
    $where .= " AND (6371 * acos(
                    ...
                    * cos(radians(b.KT_LATITUDE)) 
                    * cos(radians(b.KT_LONGTITUDE) - radians(:longitude)) 
                    + sin(radians(:latitude)) 
                    * sin(radians(b.KT_LATITUDE))
                )) <= :distance";
    $params[':latitude'] = $latitude;
    $params[':longitude'] = $longitude;
    $params[':distance'] = $distance_search;
}
...
// Lấy tổng số phòng thoả mãn điều kiện (sử dụng Haversine để lọc sơ bộ)
...
if ($searchTriggered && !empty($latitude) && !empty($longitude) && !empty($distance_search)) {
    ...
    foreach ($rooms as $room) {
        if (!empty($room['KT_LATITUDE']) && !empty($room['KT_LONGTITUDE'])) {
            // Gọi API OSRM với tọa độ (lưu ý: tham số API theo thứ tự: kinh, vĩ)
            ...
            if ($response !== false) {
                ...
                if (isset($data['routes'][0]['distance'])) {
                    ...
                    // Lưu khoảng cách đường đi nếu cần hiển thị (tùy chọn)
    ...
    // Cập nhật lại danh sách phòng theo khoảng cách đường đi thực tế
    $rooms = $filteredRooms;
}
?>
...
<div class="container-fluid">
    ...
    <div class="row">
        ...
        <form method="GET" action="your_search_script.php" class="needs-validation" novalidate>
            <div class="row">
                <div class="col">
            ...
            <div class="form-group mb-3">
                ...
                <!-- Thêm ô input hiển thị địa chỉ chi tiết -->
                <!-- <div class="row mt-2">
                    <div class="col">
                        <input type="text" class="form-control" id="address" name="address" placeholder="Địa chỉ chi tiết" readonly value="<?= htmlspecialchars($address) ?>">
                    </div>
            ...
            <!-- Thêm tiêu chí: Số người ở tối đa và Giá thuê tối đa -->
            <div class="row">
                <div class="col">
                    <div class="form-group mb-3">
                ...
                <nav aria-label="Page navigation">
                    <ul class="pagination justify-content-center">
                        <li class="page-item <?= ($page == 1) ? 'disabled' : '' ?>">
                            <a class="page-link btn btn-primary" href="?page=1<?= ($searchTriggered ? '&owner-search='.$owner_search.'&khu-tro-search='.$khu_tro_search.'&latitude='.$latitude.'&longitude='.$longitude.'&distance-search='.$distance_search.'&max-people='.$max_people.'&max-price='.$max_price.'&submit=1' : '') ?>">First</a>
                        </li>
                        <li class="page-item <?= ($page == 1) ? 'disabled' : '' ?>">
                            <a class="page-link btn btn-primary" href="?page=<?= max(1, $page - 1) ?><?= ($searchTriggered ? '&owner-search='.$owner_search.'&khu-tro-search='.$khu_tro_search.'&latitude='.$latitude.'&longitude='.$longitude.'&distance-search='.$distance_search.'&max-people='.$max_people.'&max-price='.$max_price.'&submit=1' : '') ?>">«</a>
                        </li>
                        <?php
                        $range = 2;
                        ...
                        for ($i = $start; $i <= $end; $i++): ?>
                            <li class="page-item <?= ($i == $page) ? 'active' : '' ?>">
                                <a class="page-link btn btn-primary" href="?page=<?= $i ?><?= ($searchTriggered ? '&owner-search='.$owner_search.'&khu-tro-search='.$khu_tro_search.'&latitude='.$latitude.'&longitude='.$longitude.'&distance-search='.$distance_search.'&max-people='.$max_people.'&max-price='.$max_price.'&submit=1' : '') ?>"><?= $i ?></a>
                            </li>
                        <?php endfor; ?>
                        <li class="page-item <?= ($page == $total_pages) ? 'disabled' : '' ?>">
                            <a class="page-link btn btn-primary" href="?page=<?= min($total_pages, $page + 1) ?><?= ($searchTriggered ? '&owner-search='.$owner_search.'&khu-tro-search='.$khu_tro_search.'&latitude='.$latitude.'&longitude='.$longitude.'&distance-search='.$distance_search.'&max-people='.$max_people.'&max-price='.$max_price.'&submit=1' : '') ?>">»</a>
                        </li>
                        <li class="page-item <?= ($page == $total_pages) ? 'disabled' : '' ?>">
                            <a class="page-link btn btn-primary" href="?page=<?= $total_pages ?><?= ($searchTriggered ? '&owner-search='.$owner_search.'&khu-tro-search='.$khu_tro_search.'&latitude='.$latitude.'&longitude='.$longitude.'&distance-search='.$distance_search.'&max-people='.$max_people.'&max-price='.$max_price.'&submit=1' : '') ?>">Last</a>
                        </li>
                    </ul>
                </nav>
            </div>
                ...
                <div id="map" style="width: 100%; height: 990px;">
                    ...
                    <script>
                        ...
                        if (document.getElementById('map')) {
                            ...
                            // Định nghĩa icon marker màu cam để nổi bật
                            ...
                            // Nếu cần hiển thị marker đã chọn từ modal (giữ lại chức năng này)
                            ...
                            // Biến lưu polyline của tuyến đường, để có thể xóa đi khi hiển thị tuyến đường mới
                            window.routeLayer = null;

                            // (Nếu bạn vẫn muốn hiển thị các marker của danh sách phòng)
                            ...
                            // Lắng nghe sự kiện click của các nút "Chỉ đường" trong danh sách
                            document.querySelectorAll('.direction-btn').forEach(function(button) {
                                button.addEventListener('click', function() {
                                    // Lấy thông tin khu trọ từ data-attributes
                                    ...
                                    // Lấy tọa độ điểm khởi hành từ các input (đã chọn trên modal map)
                                    ...
                                    fetch(osrmUrl)
                                        ...
                                        .then(data => {
                                            if (data.routes && data.routes.length > 0) {
                                                ...
                                                // Vẽ tuyến đường mới trên bản đồ
                                                window.routeLayer = L.geoJSON(route.geometry, {
                                                    style: { color: 'blue', weight: 4 }
                                                }).addTo(map1);
                                                
                                                // Hiển thị popup tại điểm đến với thông tin chi tiết
                                                L.popup()
