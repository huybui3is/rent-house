...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['add_phong'])) {
    ...
    $uploadBaseDir = "/xampp/htdocs/htqlnt/ChuTro/uploadss/";  // Điều chỉnh đường dẫn này theo cấu trúc của bạn
...
if (isset($_GET['delete_phong'])) {
    ...
    try {
        ...
        // Cập nhật lịch sử thuê: chỉ cập nhật bản ghi mới nhất của mỗi phòng nếu chưa có TTP_MA = '02'
        ...
        // Vì chỉ có placeholders trong subquery nên truyền mảng $maPhongList một lần
        ...
        $conn->commit();
        $message = "Xóa mềm nhiều phòng, lịch sử thuê và phiếu thuê thành công!";
        $messageType = "success";
    } catch (PDOException $e) {
        $conn->rollBack();
        $message = "Lỗi hệ thống: " . $e->getMessage();
